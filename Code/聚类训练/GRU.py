...
SAVE_ROOT = Path("/root/autodl-tmp/airport_project/data/xiao/check/cluster/gru")
...
# GRU / LSTM / CNN / BiLSTM 这类 Keras 模型不建议 Optuna 并行
OPTUNA_N_JOBS = 1
...
def build_gru_model(input_shape, gru_units_1, gru_units_2, dropout_rate, learning_rate):
    model = Sequential()
...
def train_one_cluster(class_name, train_path, valid_path):
    ...
    model_save_path = cluster_save_dir / "best_gru_model.keras"
    ...
    def objective(trial):
        ...
        try:
            ...
            y_pred_scaled = model.predict(X_valid_gru, verbose=0).reshape(-1, 1)
    ...
    y_pred_scaled = best_model.predict(X_valid_gru, verbose=0).reshape(-1, 1)
    y_pred = scaler_y.inverse_transform(y_pred_scaled).reshape(-1)
    y_pred = np.maximum(y_pred, 0)
...
def main():
    ...
    summary_csv_path = SAVE_ROOT / "gru_all_classes_all_clusters_metrics.csv"
