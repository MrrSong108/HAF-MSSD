...
SAVE_ROOT = Path("/root/autodl-tmp/airport_project/results/result_cluster/bilstm_cluster_models_split82_plus_validation")
...
# BiLSTM / LSTM / GRU / CNN 不建议 Optuna 并行
OPTUNA_N_JOBS = 20
...
def build_bilstm_model(input_shape, lstm_units_1, lstm_units_2, dropout_rate, learning_rate):
    model = Sequential()
...
def train_one_cluster(class_name, train_path, valid_path):
    ...
    model_save_path = cluster_save_dir / "best_bilstm_model.keras"
    ...
    # 当前每一行已经是一个样本，因此时间步长设为 1
    X_train_bilstm = X_train.reshape(X_train.shape[0], 1, X_train.shape[1])
    X_valid_bilstm = X_valid.reshape(X_valid.shape[0], 1, X_valid.shape[1])
    ...
    def objective(trial):
        ...
        try:
            ...
            y_pred_scaled = model.predict(X_valid_bilstm, verbose=0).reshape(-1, 1)
    ...
    y_pred_scaled = best_model.predict(X_valid_bilstm, verbose=0).reshape(-1, 1)
    y_pred = scaler_y.inverse_transform(y_pred_scaled).reshape(-1)
    y_pred = np.maximum(y_pred, 0)
...
def main():
    ...
    summary_csv_path = SAVE_ROOT / "bilstm_all_classes_all_clusters_metrics.csv"
