...
OUTPUT_DIR = PROJECT_DIR / "data/TCFM/tcfm_euclidean_rf_optuna"
...
def suggest_rf_params(trial):
    params = {
        ...
        # 以 18 为中心，搜索更强或更弱的分裂约束
        "min_samples_split": trial.suggest_int("min_samples_split", 2, 40),

        # 以 9 为中心，控制叶节点最小样本
        "min_samples_leaf": trial.suggest_int("min_samples_leaf", 1, 20),

        # 以 0.5 为中心，搜索每次分裂使用的特征比例
...
def optimize_rf_for_cluster(
    ...
    """
    在单个簇内部使用 Optuna 搜索 Random Forest 参数。
    目标函数：验证集 RMSE 最小。
    """
    ...
    study = optuna.create_study(
        direction="minimize",
        sampler=sampler,
        study_name=f"rf_cluster_{cluster_id}"
    )
...
print("正在训练全局兜底 RF 模型...")

global_model = create_rf_model(BASE_RF_PARAMS)
...
for cluster_id in range(best_k):
    ...
    if n_train_cluster < MIN_CLUSTER_SAMPLES:
        print(
            ...
            f"不单独训练，使用全局 RF 模型作为兜底。"
        )

        cluster_models[cluster_id] = global_model
        cluster_best_params[cluster_id] = BASE_RF_PARAMS.copy()
        ...
        cluster_optuna_records.append({
            ...
            "best_rmse": None,
            "best_params": json.dumps(BASE_RF_PARAMS, ensure_ascii=False)
        })
    ...
    if n_valid_cluster == 0:
        print(
            ...
            f"无法进行 Optuna 验证调参，使用基础 RF 参数训练。"
        )

        X_c_train = X_train.iloc[train_idx]
        y_c_train = y_train.iloc[train_idx]

        model = create_rf_model(BASE_RF_PARAMS)
        model.fit(X_c_train, y_c_train)
    ...
    # Optuna 搜索 RF 参数
    # -----------------------------
    best_params, best_rmse, study = optimize_rf_for_cluster(
        X_c_train=X_c_train,
        y_c_train=y_c_train,
        X_c_valid=X_c_valid,
        y_c_valid=y_c_valid,
...
joblib.dump(cluster_models, OUTPUT_DIR / "cluster_rf_models.pkl")
joblib.dump(global_model, OUTPUT_DIR / "global_rf_model.pkl")
joblib.dump(cluster_best_params, OUTPUT_DIR / "cluster_rf_best_params.pkl")
...
config = {
    ...
    "rf_search_space": {
        ...
        "max_depth": "8 to 32",
        "min_samples_split": "2 to 40",
        "min_samples_leaf": "1 to 20",
        "max_features": "0.3 to 1.0, step 0.1",
        "bootstrap": "[True, False]"
    },
    "model": f"TCFM-{CLUSTER_METRIC}-RF-Optuna",
    "description": (
        ...
        "The prediction stage uses all valid multi-source features and Random Forest models optimized by Optuna within each cluster."
    )
}
