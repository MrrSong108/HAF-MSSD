...
OUTPUT_DIR = PROJECT_DIR / "data/TCFM/tcfm_dtw_xgb"
...
# =========================================================
# 7. 使用原文思想：时间序列聚类特征格式转换
# =========================================================
"""
原文聚类思想：
1. 使用时间序列片段作为聚类对象
2. 使用 Euclidean distance 或 DTW distance 衡量序列相似性
3. 在聚类基础上进行分组预测

在你的数据中，queue_countpassed5、queue_countpassed10、...、queue_countpassed24h
可以理解为目标序列的历史窗口特征，因此这里将其转换为时间序列格式：
...
这样才能使用 TimeSeriesKMeans。
"""

# 可选："euclidean" 或 "dtw"
# euclidean：速度快，适合作为主要实验
# dtw：更贴近原文的时间形态匹配思想，但计算更慢
CLUSTER_METRIC = "dtw"
# dtw euclidean
...
# =========================================================
# 8. 使用 TimeSeriesKMeans 搜索最优聚类数 K
# =========================================================
...
for k in range(K_MIN, K_MAX + 1):
    ...
    # DBI 本质上基于欧氏空间距离。
    # 对 euclidean 聚类比较严格；
    # 对 dtw 聚类时，这里将时间序列展平成二维后近似计算 DBI。
...
config = {
    ...
    "model": f"TCFM-{CLUSTER_METRIC}-XGBoost",
    "description": "Time-series clustering-based fusion forecasting model using TimeSeriesKMeans with Euclidean or DTW distance."
}
