...
SAVE_ROOT = Path(
    os.getenv(
        "SAVE_ROOT",
        "outputs/xgboost_cluster_models",
    )
)
...
# Keep Optuna single-threaded by default to avoid excessive CPU/GPU usage.
OPTUNA_N_JOBS = int(os.getenv("OPTUNA_N_JOBS", "1"))

# XGBoost parallel jobs.
XGB_N_JOBS = int(os.getenv("XGB_N_JOBS", "-1"))

# For public GitHub code, CPU is safer as the default.
# If GPU is available, run:
#   XGBOOST_DEVICE=cuda python your_script.py
...
# ==================================================
# 4. XGBoost device configuration
# ==================================================
def get_xgb_device_params():
    """
    XGBoost 2.x uses tree_method='hist' with device='cuda' for GPU training.
    Older versions use tree_method='gpu_hist' and predictor='gpu_predictor'.

    CPU is used by default for better compatibility in public repositories.
    """
...
# ==================================================
# 5. Find valid cluster training files
# ==================================================
...
def train_one_cluster(class_name, train_path, validation_path, test_path):
    ...
    model_save_path = cluster_save_dir / "best_xgboost_model.pkl"
    ...
    # ==================================================
    best_model = create_xgb_model(final_params)
    ...
    with open(params_save_path, "w", encoding="utf-8") as f:
        json.dump(params_to_save, f, ensure_ascii=True, indent=4, default=str)
...
def main():
    ...
    summary_csv_path = SAVE_ROOT / "cluster_xgboost.csv"
