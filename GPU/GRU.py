...
from keras.layers import GRU, Dense, Bidirectional, Conv1D, Flatten, Dropout
...
# GRU 模型
def build_gru_model(input_shape, gru_units_1, gru_units_2, dropout_rate):
    model = Sequential()
    model.add(GRU(gru_units_1, activation='relu', input_shape=input_shape, return_sequences=True))
    model.add(Dropout(dropout_rate))
    model.add(GRU(gru_units_2, activation='relu'))
    model.add(Dropout(dropout_rate))
...
def objective_gru(trial):
    # 超参数空间
    gru_units_1 = trial.suggest_int('gru_units_1', 32, 512, step=32)
    gru_units_2 = trial.suggest_int('gru_units_2', 16, 256, step=16)
    dropout_rate = trial.suggest_float('dropout_rate', 0.0, 0.7)
    batch_size = trial.suggest_int('batch_size', 16, 128, step=16)
    epochs = trial.suggest_int('epochs', 50, 200, step=10)
    patience = trial.suggest_int('patience', 5, 20)
    # 构建GRU模型
    model = build_gru_model((X_train.shape[1], X_train.shape[2]), gru_units_1, gru_units_2, dropout_rate)
...
# 开始优化
study_gru.optimize(objective_gru, n_trials=30, n_jobs=1)  # 进行50次优化

# 输出最优结果
print(f"最优 RMSE: {study_gru.best_value}")
print(f"最佳超参数: {study_gru.best_params}")

# 使用最佳超参数训练 GRU 模型
best_params = study_gru.best_params
best_gru_model = build_gru_model((X_train.shape[1], X_train.shape[2]),
                                 best_params['gru_units_1'],
                                 best_params['gru_units_2'],
                                 best_params['dropout_rate'])
early_stopping = EarlyStopping(monitor='val_loss',
                               patience=best_params['patience'],
                               restore_best_weights=True)
best_gru_model.fit(X_train, y_train,
...
# 使用最佳模型进行预测
y_pred_scaled = best_gru_model.predict(X_test)
...
# 验证集
# y_validation_pred = best_gru_model.predict(X_validation)
#
# # 反归一化
...
test_df = pd.DataFrame({
    ...
    'GRU': y_pred_original.flatten()
})
test_df.to_csv(r'D:\真能毕业吗\机场客流量\机场客流量预测投稿\JATM航空运输管理\对比模型返回结果\test_gru.csv', index=False)

# test_df_validation = pd.DataFrame({
#     'Actual': y_validation_inv.flatten(),
#     'GRU': y_validation_original.flatten()
# })
# test_df_validation.to_csv(r'D:\真能毕业吗\机场客流量\机场客流量预测投稿\validation_gru.csv', index=False)
