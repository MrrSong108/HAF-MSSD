...
# 读取数据
train = pd.read_csv(r"/root/autodl-tmp/airport_project/data/train.csv")
test = pd.read_csv(r"/root/autodl-tmp/airport_project/data/test.csv")
validation = pd.read_csv(r"/root/autodl-tmp/airport_project/data/validation.csv")
...
# biLSTM 模型
def build_bilstm_model(input_shape, lstm_units_1, lstm_units_2, dropout_rate):
    model = Sequential()
    model.add(Bidirectional(LSTM(lstm_units_1, activation='relu', return_sequences=True), input_shape=input_shape))
    model.add(Dropout(dropout_rate))
    model.add(Bidirectional(LSTM(lstm_units_2, activation='relu')))
    model.add(Dropout(dropout_rate))
...
def objective_bilstm(trial):
    ...
    # 构建biLSTM模型
    model = build_bilstm_model((X_train.shape[1], X_train.shape[2]), lstm_units_1, lstm_units_2, dropout_rate)
...
# 开始优化
study_bilstm.optimize(objective_bilstm, n_trials=30, n_jobs=1)  # 进行50次优化

# 输出最优结果
print(f"最优 RMSE: {study_bilstm.best_value}")
print(f"最佳超参数: {study_bilstm.best_params}")

# 使用最佳超参数训练 biLSTM 模型
best_params = study_bilstm.best_params
best_bilstm_model = build_bilstm_model((X_train.shape[1], X_train.shape[2]),
...
best_bilstm_model.fit(X_train, y_train,
...
# 使用最佳模型进行预测
y_pred_scaled = best_bilstm_model.predict(X_validation)

# 反归一化
y_pred = scaler_y.inverse_transform(y_pred_scaled.reshape(-1, 1))
y_pred_original = np.maximum(y_pred, 0)
y_test_inv = scaler_y.inverse_transform(y_validation)
...
# 验证集
# y_validation_pred = best_bilstm_model.predict(X_validation)
# 
# # 反归一化
...
#     'biLSTM': y_pred_original.flatten()
# })
# test_df.to_csv(r'D:\真能毕业吗\机场客流量\机场客流量预测投稿\JATM航空运输管理\对比模型返回结果\test_bilstm.csv', index=False)

# test_df_validation = pd.DataFrame({
#     'Actual': y_validation_inv.flatten(),
#     'biLSTM': y_validation_original.flatten()
# })
# test_df_validation.to_csv(r'D:\真能毕业吗\机场客流量\机场客流量预测投稿\validation_bilstm.csv', index=False)
# pdb.set_trace()
# # 绘制结果图
