...
# X_train, X_test, y_train, y_test = train_test_split(features_train, train_Y, test_size=0.2, random_state=42)
...
# start_time = time.time()
# X_train = X_train_trans.reshape(X_train.shape[0], 1, X_train.shape[1])
# X_test = X_test_trans.reshape(X_test.shape[0], 1, X_test.shape[1])
# CNN 模型
def build_cnn_model(input_shape, filters_1, filters_2, kernel_size, dropout_rate):
    model = Sequential()
    model.add(Conv1D(filters=filters_1, kernel_size=1, activation='relu', input_shape=input_shape))
    model.add(Dropout(dropout_rate))
    model.add(Conv1D(filters=filters_2, kernel_size=1, activation='relu'))
    model.add(Dropout(dropout_rate))
    model.add(Flatten())  # 展平层，将卷积结果展平以便全连接层输入
...
def objective_cnn(trial):
    ...
    filters_2 = trial.suggest_int('filters_2', 8, 64, step=8)
    ...
    # 构建CNN模型
    model = build_cnn_model((X_train.shape[1], X_train.shape[2]), filters_1, filters_2, kernel_size, dropout_rate)
...
#
# # 开始优化
# study_cnn.optimize(objective_cnn, n_trials=30, n_jobs=1)  # 进行50次优化
#
# # 输出最优结果
# print(f"最优 RMSE: {study_cnn.best_value}")
# print(f"最佳超参数: {study_cnn.best_params}")
# start_time = time.time()
# # 使用最佳超参数训练 CNN 模型
# best_params = study_cnn.best_params
# best_cnn_model = build_cnn_model((X_train.shape[1], X_train.shape[2]),
...
best_cnn_model.fit(X_train, y_train,
                   ...
                   callbacks=[early_stopping])
# 使用最佳模型进行预测
y_pred_scaled = best_cnn_model.predict(X_test)
...
#
# # 验证集
# y_validation_pred = best_cnn_model.predict(X_validation)
#
# # 反归一化
...
test_df = pd.DataFrame({
    ...
    'CNN': y_pred_original.flatten()
})
test_df.to_csv(r'D:\真能毕业吗\机场客流量\机场客流量预测投稿\JATM航空运输管理\对比模型返回结果\test_cnn.csv', index=False)
#
# test_df_validation = pd.DataFrame({
#     'Actual': y_validation_inv.flatten(),
#     'CNN': y_validation_original.flatten()
# })
# test_df_validation.to_csv(r'D:\真能毕业吗\机场客流量\机场客流量预测投稿\validation_cnn.csv', index=False)
#
# # 绘制结果图
