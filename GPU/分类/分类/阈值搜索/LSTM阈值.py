...
save_dir = "/root/autodl-tmp/airport_project/results/result_classifier/result_lstm_classifier"
...
study_lstm_cls.optimize(
    objective,
    n_trials=50,
    n_jobs=1
)

print(f"\nOptuna 最优验证集 Balanced Accuracy: {study_lstm_cls.best_value:.4f}")
print("最佳超参数:")
print(study_lstm_cls.best_params)

print("\nOptuna 最优 trial 对应阈值:")
print(study_lstm_cls.best_trial.user_attrs)


# =========================
# 11. 保存最佳参数
# =========================
best_params = study_lstm_cls.best_params

with open(
    os.path.join(save_dir, "best_lstm_classifier_params.json"),
    "w",
    encoding="utf-8"
) as f:
...
threshold_info = {
    ...
    "optuna_best_value": float(study_lstm_cls.best_value),
    "optuna_best_trial_threshold": float(
        study_lstm_cls.best_trial.user_attrs.get("best_threshold", -1)
    )
}
...
# =========================
best_model.save(
    os.path.join(save_dir, "lstm_classifier.keras")
)
