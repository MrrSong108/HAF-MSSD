...
# CNN input format: [num_samples, num_features, 1]
# Conv1D performs convolution along the feature dimension.
...
# =========================
# 6. CNN model construction function
# =========================
def build_model(
    input_shape,
    filters_1,
...
print("Best validation RMSE:", study_cnn.best_value)
print("Best hyperparameters:", study_cnn.best_params)

best_params = study_cnn.best_params

with open(OUTPUT_DIR / "best_cnn_params.json", "w", encoding="utf-8") as f:
    json.dump(best_params, f, ensure_ascii=True, indent=4)


# =========================
# 9. Retrain CNN model with the best hyperparameters
# =========================
K.clear_session()
gc.collect()
...
# =========================
# 11. Error-based classification label generation
# =========================
...
# =========================
# 12. Save model, scalers, and generated results
# =========================
best_model.save(OUTPUT_DIR / "best_cnn_model.keras")
...
X_validation_data = pd.DataFrame(
    X_validation.reshape(X_validation.shape[0], X_validation.shape[1]),
