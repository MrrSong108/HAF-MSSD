...
# }
#
# SAVE_ROOT = Path("/root/autodl-tmp/airport_project/data/xiao/check/cluster/cnn")
CLASS_DIRS = {
    "class_0_challenging": Path("/root/autodl-tmp/airport_project/results/result_cluster/result_rf_kmeans/class_0_challenging"),
    "class_1_friendly": Path("/root/autodl-tmp/airport_project/results/result_cluster/result_rf_kmeans/class_1_friendly"),
}

SAVE_ROOT = Path("/root/autodl-tmp/airport_project/results/result_cluster/cnn_cluster_models_split82_plus_validation")
...
def build_cnn_model(input_shape, filters_1, filters_2, dropout_rate, learning_rate):
    model = Sequential()
...
def train_one_cluster(class_name, train_path, valid_path):
    ...
    model_save_path = cluster_save_dir / "best_cnn_model.keras"
    ...
    X_valid_cnn = X_valid.reshape(X_valid.shape[0], 1, X_valid.shape[1])
    ...
    def objective(trial):
        ...
        y_pred_scaled = model.predict(X_valid_cnn, verbose=0).reshape(-1, 1)
    ...
    y_pred_scaled = best_model.predict(X_valid_cnn, verbose=0).reshape(-1, 1)
    y_pred = scaler_y.inverse_transform(y_pred_scaled).reshape(-1)
    y_pred = np.maximum(y_pred, 0)
    ...
    split_info = {
        ...
        "cnn_input_shape": list(input_shape),
        "kernel_size": 1
    }
...
def main():
    ...
    summary_csv_path = SAVE_ROOT / "cnn_all_classes_all_clusters_metrics.csv"
